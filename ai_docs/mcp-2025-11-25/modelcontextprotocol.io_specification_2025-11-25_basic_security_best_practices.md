...
Third-Party AuthZ ServerMCP ServerUser's BrowserMCP ClientThird-Party AuthZ ServerMCP ServerUser's BrowserMCP Client1\. Client Registration (Dynamic)2\. Authorization RequestNot previously approvedalt\[Check MCP Server Consent\]"Allow \[Client Name\] to access \[Third-Party API\]?"3\. Forward to Third-PartyUse static client\_id for third-partyRegister with redirect\_uriclient\_idOpen MCP server authorization URLGET /authorize?client\_id=...&redirect\_uri=...Check consent for this client\_idShow MCP server-owned consent pagePOST /consent (approve)Store consent decision for client\_idRedirect to third-party /authorizeAuthorization request (static client\_id)User authenticates & consentsRedirect with auth codeCallback with third-party codeExchange code for token (using static client\_id)Redirect to client's registered redirect\_uri
