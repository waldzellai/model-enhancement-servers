...
class VisualReasoningServer {
    ...
    if (operationType === 'transform' && operation.transformationType && elements) {
      for (const element of elements) {
        ...
        switch (operation.transformationType) {
          ...
          case 'rotate':
            if (element.properties.rotation !== undefined) targetElement.properties.rotation = element.properties.rotation;
            break;
  ...
  public processVisualOperation(input: unknown): { content: Array<{ type: string; text: string }>; isError?: boolean } {
    try {
      ...
      const asciiDiagram = this.renderAsciiDiagram(
        validatedInput.diagramId, 
        validatedInput.diagramType
      );
