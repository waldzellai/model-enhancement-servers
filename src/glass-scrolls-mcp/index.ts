...
async function writeMemo(filePath: string, memo: string): Promise<void> {
...
await yargs(hideBin(process.argv))
  ...
  .command(
    'demo',
    'Run demo plan, capture trace, and write outputs',
    () => {},
    async () => {
      const plan = demoPlan();
  ...
  .command(
    'replay',
    'Replay the plan from recorded trace without calling tools',
    () => {},
    async () => {
      const plan = demoPlan();
