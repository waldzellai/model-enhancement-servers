...
export type TxnAction = 'start' | 'resume' | 'close'; // Keep for clarity if used elsewhere
...
function calculateExpiry(ttlSeconds: number): string {
  ...
  try {
    ...
    switch (action) {
      case 'start': {
        ...
        if (!setResult) throw new Error('Failed to set initial state in Redis.');
        responsePayload = {
          status: 'pending',
          token: newToken,
          payload: payload ?? {},
