...
class CollaborativeReasoningServer {
  ...
  private updateRegistries(data: CollaborativeReasoningData): void {
    ...
    if (data.disagreements && data.disagreements.length > 0) {
      ...
      for (const disagreement of data.disagreements) {
        ...
        const existingIndex = this.disagreementTracker[sessionId].findIndex(
          d => d.topic === disagreement.topic
        );
  ...
  public processCollaborativeReasoning(input: unknown): { content: Array<{ type: string; text: string }>; isError?: boolean } {
    try {
